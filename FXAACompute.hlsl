...
groupshared float4 image_row[1024];

static float3 kernel1D = float3(0.27901f, 0.44198f, 0.27901f);

[numthreads(1024, 1, 1)]
void FXAA_main(uint3	dispaThreadID	: SV_DispatchThreadID,	// Global position
	uint3	groupThreadID : SV_GroupThreadID,		// Group position
	uint	groupIndex : SV_GroupIndex)
{
	float2 pos = dispaThreadID.xy;
