...
bool FrustumHandler::checkVisible(DirectX::XMVECTOR boxMin, DirectX::XMVECTOR boxMax) {
	...
	for (int i = 0; i < 6; i++)
	{
		...
		for (int j = 0; j < 8; j++)
		{
			...
			float tempDot = planes[i].a*DirectX::XMVectorGetX(diagonal[j]) + planes[i].b*DirectX::XMVectorGetY(diagonal[j])+ planes[i].c*DirectX::XMVectorGetY(diagonal[j]);
			if (tempDot > highDot)
			{
				highDot = tempDot;
		...
		DirectX::XMVECTOR point = { xMiddle - DirectX::XMVectorGetX(mostResembling),yMiddle - DirectX::XMVectorGetY(mostResembling),zMiddle - DirectX::XMVectorGetY(mostResembling),1 };
